...
let path = {
	...
	src: {
		...
		js: [src_folder + "/js/app.js", src_folder + "/js/vendors.js", src_folder + "/js/fetch.js", src_folder + "/js/select.js", src_folder + "/js/ticket.js"],
...
function jsBuild() {
	...
	let fetchPath = path.build.js + 'fetch.min.js';
	let selectPath = path.build.js + 'select.min.js';
	del(appPath);
	del(vendorsPath);
	del(fetchPath);
	del(selectPath);
			...
			'append': {
				...
				'to': [
					...
					['image', '%TS%'],
					{
						...
						'value': '%DT%',
						'cover': 1,
						'files': ['app.min.js', 'vendors.min.js', 'fetch.min.js', 'select.min.js'] // Array [{String|Regex}] of explicit files to append to
					}
				]
			},
			'output': {
